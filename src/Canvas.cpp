...
void Canvas::sendToFront(Shape* selectedShape){
    for (unsigned int i = 0; i < shapes.size(); i++ ){
...
void Canvas::sendToBack(Shape* selectedShape){
    for (unsigned int i = 0; i < shapes.size(); i++ ){
        if (shapes[i] == selectedShape){
            shapes.erase(shapes.begin() + i);
...
void Canvas::increaseSize(Shape* selectedShape){
    if (selectedShape){
        selectedShape->changeSize(1);
    }
}

void Canvas::decreaseSize(Shape* selectedShape){
    if (selectedShape){
        selectedShape->changeSize(-1);
    }
}